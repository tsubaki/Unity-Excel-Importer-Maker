...
public class ExcelImporterMaker : EditorWindow
{
	...
	void OnGUI ()
	{
		...
		int preSelectedSheet = selectedSheetFormat;
		selectedSheetFormat =EditorGUILayout.Popup("export scriptableobject format", selectedSheetFormat, sheetNameList.ToArray());
		if( preSelectedSheet != selectedSheetFormat)
		{
			SetupTypelist(selectedSheetFormat);
		}
		...
		var item = sheetList.FindAll( (obj)=> obj.isEnable == true);
		if( item.Count > 1)
		{
		...
		if (GUILayout.Button ("create")) {
			ISheet sheet = book.GetSheetAt (selectedSheetFormat);
			EditorPrefs.SetString (s_key_prefix + fileName  + sheet.SheetName + ".className", className);
			ExportEntity ();
			ExportImporter ();
	...
	private int selectedSheetFormat;
	...
	/// <summary>
	/// Exports the excel to assetbundle.
	/// </summary>
	...
	static void ExportExcelToAssetbundle ()
	{
		foreach (Object obj in Selection.objects) {
			...
			using (FileStream stream = File.Open (window.filePath, FileMode.Open, FileAccess.Read)) {
				...
				window.SetupSheetList();
				window.SetupTypelist(0);
	...
	void SetupSheetList()
	{
		...
		for (int i = 0; i < book.NumberOfSheets; ++i) {
			...
			sht.isEnable = EditorPrefs.GetBool (s_key_prefix + fileName + ".sheet." + sht.sheetName, true);
			sheetNameList.Add(s.SheetName);
			sheetList.Add (sht);
		}
	}
	...
	void ExportImporter ()
	{
		...
		importerTemplate = importerTemplate.Replace ("$ExportTemplate$", fileName + "_" + className + "_importer");
			
		Directory.CreateDirectory ("Assets/Terasurware/Classes/Editor/");
		File.WriteAllText ("Assets/Terasurware/Classes/Editor/" + fileName + "_" + className + "_importer.cs", importerTemplate);
	}
